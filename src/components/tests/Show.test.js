...
test('renders same number of options seasons are passed in', () => {
  ...
  expect(seasonOption.length).toEqual(testShow.seasons.length);
});
...
test('handleSelect is called when an season is selected', () => {
  ...
  //Act
  const seasonsSelect = screen.queryByTestId(/seasons-select/i);
  userEvent.selectOptions(seasonsSelect, '1');
  userEvent.selectOptions(seasonsSelect, '0');
...
test('component renders when no seasons are selected and when rerenders with a season passed in', () => {
  ...
  expect(episodesContainer).not.toBeNull();
});
